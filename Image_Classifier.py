...
# Create the model as a sequential type so that we can add layers in oder
model = Sequential()
# Add the first convultion to output a feature map
...
# kernal_constraint:  see above
model.add(Dense(units=512, activation='relu', kernel_constraint=maxnorm(3)))
# Dropout layer to ignore some neurons during training wich improves model reliability
